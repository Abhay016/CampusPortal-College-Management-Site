...
from app.models import Course, Session_Year, CustomUser, Student, Staff, Subject, Staff_Notification, Staff_leave, Staff_feedback, Student_Notification, Student_feedback, Student_leave, Attendance, AttendanceReport
...
def UPDATE_COURSE(request):
    if request.method == "POST":
        ...
        course.name=name
        course.save()
        messages.success(request, 'Courses are Successfully Updated')
        return redirect('view_course')
...
def ADD_SUBJECT(request):
    ...
    if request.method == "POST":
        ...
        subject.save()
        messages.success(request,'Subjects Are Successfully Created ')
...
def STAFF_APPROVE_LEAVE(request, id):
    ...
    leave.status = 1
    leave.save()
    messages.success(request, 'Staff Leave is Approved.')
...
def STAFF_DISAPPROVE_LEAVE(request, id):
    ...
    leave.status = 2
    leave.save()
    messages.success(request, 'Staff Leave is Rejected.')
    return redirect('staff_view_leave')
...
def STUDENT_APPROVE_LEAVE(request, id):
    ...
    leave.status = 1
    leave.save()
    messages.success(request, 'Student Leave is Approved.')
...
def STUDENT_DISAPPROVE_LEAVE(request, id):
    ...
    leave.status = 2
    leave.save()
    messages.success(request, 'Student Leave is Rejected.')
    return redirect('student_view_leave')
